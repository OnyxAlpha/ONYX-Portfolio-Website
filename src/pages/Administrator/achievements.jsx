...
export default function AdminAchievements({ sidebarToggle }) {
  const [achievements, setAchievements] = useState([]);
    ...
    const addAchievement = async (event) => {
      ...
      const data = await fetch(`${process.env.REACT_APP_PORTFOLIO_API}/administratorachievements`,{
        method: "POST",
        body: formData, 
      });
    if (data.status === 201) {
      ...
      const getAchievements = () =>{
        const url = new URL (`${process.env.REACT_APP_PORTFOLIO_API}/administratorachievements`)
    ...
    return (
        ...
        >
            ...
            <div className="flex gap-3 px-2">
              <i onClick={()=> updateAchievement(post)}className="fa-solid fa-pen-to-square p-2 text-xl text-violet-500 hover:text-violet-700 focus:text-violet-700"></i>
              <div className="h-5 bg-gray-500 w-0.5 mt-3"></div>
              <i onClick={()=> deleteAchievement()} class="fa-solid fa-trash p-2 text-xl text-rose-600 hover:text-rose-700 focus:text-rose-700"></i>
            </div>
          </div>
          <hr />
